...
- **[v1.x] `SettingsPage.tsx` per-tab split** — 823-line tabbed component → `pages/admin/settings/{BrandingTab,EmailTab,AnalyticsTab,…}.tsx`. Each tab owns its UI subtree, with shared form-state lifted to the parent.
- **[v1.x] `DataSeeder.cs` per-concern split** — 967-line seeder → `IdentityAndSystemSeeder` (roles + system user + admin + site settings + system pages + email templates), `SamplePeopleAndCommunitySeeder` (leaders + groups + classes + members + prayer + connect cards + volunteers + broadcasts), and `SampleContentSeeder` (sample pages + news + sermons + events + blog + documents). Demo content gated by `Seed:Demo` flag.
